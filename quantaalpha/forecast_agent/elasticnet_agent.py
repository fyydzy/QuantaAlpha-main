"""ElasticNet 预测后端（与 Lasso 同一套 Forecast Agent 流程；训练仍为 ElasticNetCV）。"""
...
# 与旧脚本 elasticnet 一致：联合搜索 alpha 与 l1_ratio
_L1_RATIO_GRID = (0.2, 0.5, 0.8, 0.9, 0.95, 1.0)


@dataclass(frozen=True)
class ElasticnetHyperParams:
    context_len: int = 111
...
def _build_feedback_message(metrics: dict[str, float], params: ElasticnetHyperParams) -> str:
    notes: list[str] = []
    if metrics["smape"] > 35:
    ...
    return "；".join(notes) if notes else "ElasticNet 当前配置较稳定"
...
def _load_feature_frame(path: str) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"文件缺少 ElasticNet 所需列: {sorted(missing)}")
...
def _prepare_train_and_future(
    ...
    if len(future_df) != len(ctx.forecast_months):
        ...
        raise ValueError(f"ElasticNet 特征月份不完整，缺少: {missing}")

    feature_cols = [c for c in feature_df.columns if c not in {MONTH_COL, TARGET_COL}]
    return train_df, future_df, feature_cols


def _fit_elasticnet_cv(X_train: np.ndarray, y_train: np.ndarray) -> tuple[ElasticNetCV, StandardScaler]:
...
class ElasticnetEvaluator(ForecastEvaluator):
    def evaluate(self, task: ForecastTask, subjects: ForecastSubjects) -> ForecastFeedback:
        ...
        try:
            ...
            model, scaler = _fit_elasticnet_cv(X_train, y_train)
            ...
            return _failed_feedback(f"ElasticNet 推理失败: {exc}")


class FixedElasticnetStrategy(ForecastStrategy):
    def __init__(self, context_len: int = 111) -> None:
        self.context_len = int(context_len)

    def seed_subjects(self, task: ForecastTask) -> list[ForecastSubjects]:
        params = ElasticnetHyperParams(context_len=self.context_len)
        return [
            ForecastSubjects(
                params=params,
...
class AutoElasticnetForecastAgent(ForecastAgent):
    def __init__(
        self,
        context_len: int = 111,
        ...
        super().__init__(
            strategy=strategy or FixedElasticnetStrategy(context_len=context_len),
            evaluator=evaluator or ElasticnetEvaluator(),
        )
    ...
    def _format_failure(self) -> str:
        ...
        return f"ElasticNet forecast agent failed to find a valid configuration.{suffix}"
    ...
    def save_outputs(
        ...
        trace_path = output_dir / "elasticnet_search_trace.csv"
        forecast_path = output_dir / "elasticnet_forecast.csv"
        summary_path = output_dir / "elasticnet_best_summary.json"
        plot_path = output_dir / "elasticnet_forecast_plot.png"
        ...
        for idx, step in enumerate(self.trace, start=1):
            ...
            row = {
                ...
                **step.evolvable_subjects.params.to_dict(),
                ...
                "l1_ratio": (feedback.metrics.get("l1_ratio") if feedback and feedback.metrics else None),
    ...
    def _save_plot(series: pd.Series, forecast_df: pd.DataFrame, plot_path: Path) -> None:
        ...
        plt.title("Forecast Agent - Auto ElasticNet")
        plt.xlabel("Date")
        plt.ylabel("y")
        plt.legend()
        plt.tight_layout()
