"""Ridge 预测后端（与 Lasso 同一套 Forecast Agent 流程；训练仍为 StandardScaler + RidgeCV）。"""
...
# 与原 ridge 脚本一致：对数网格 + 5 折 CV
_RIDGE_ALPHAS = np.logspace(-4, 4, 200)
...
def _build_feedback_message(metrics: dict[str, float], params: RidgeHyperParams) -> str:
    notes: list[str] = []
    if metrics["smape"] > 35:
    ...
    return "；".join(notes) if notes else "Ridge 当前配置较稳定"
...
def _load_feature_frame(path: str) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"文件缺少 Ridge 所需列: {sorted(missing)}")
...
def _prepare_train_and_future(
    ...
    if len(future_df) != len(ctx.forecast_months):
        ...
        raise ValueError(f"Ridge 特征月份不完整，缺少: {missing}")

    feature_cols = [c for c in feature_df.columns if c not in {MONTH_COL, TARGET_COL}]
    return train_df, future_df, feature_cols


def _fit_ridge_cv(X_train: np.ndarray, y_train: np.ndarray) -> tuple[RidgeCV, StandardScaler]:
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X_train)
    model = RidgeCV(alphas=_RIDGE_ALPHAS, cv=5).fit(X_scaled, y_train)
    return model, scaler


class RidgeEvaluator(ForecastEvaluator):
    def evaluate(self, task: ForecastTask, subjects: ForecastSubjects) -> ForecastFeedback:
        ctx = load_task_context(task)
        params: RidgeHyperParams = subjects.params  # type: ignore[assignment]
        try:
            ...
            model, scaler = _fit_ridge_cv(X_train, y_train)
            ...
            return _failed_feedback(f"Ridge 推理失败: {exc}")


class FixedRidgeStrategy(ForecastStrategy):
    def __init__(self, context_len: int = 111) -> None:
        self.context_len = int(context_len)

    def seed_subjects(self, task: ForecastTask) -> list[ForecastSubjects]:
        params = RidgeHyperParams(context_len=self.context_len)
        return [
            ForecastSubjects(
                params=params,
...
class AutoRidgeForecastAgent(ForecastAgent):
    def __init__(
        self,
        context_len: int = 111,
        ...
        super().__init__(
            strategy=strategy or FixedRidgeStrategy(context_len=context_len),
            evaluator=evaluator or RidgeEvaluator(),
        )
    ...
    def _format_failure(self) -> str:
        ...
        return f"Ridge forecast agent failed to find a valid configuration.{suffix}"

    def refit_and_forecast(self, task: ForecastTask, params: RidgeHyperParams) -> dict[str, Any]:
        ctx = load_task_context(task)
        train_df, future_df, feature_cols = _prepare_train_and_future(ctx, params)
        X_train = train_df[feature_cols].to_numpy(dtype=float)
        y_train = train_df[TARGET_COL].to_numpy(dtype=float)
    ...
    def save_outputs(
        ...
        trace_path = output_dir / "ridge_search_trace.csv"
        forecast_path = output_dir / "ridge_forecast.csv"
        summary_path = output_dir / "ridge_best_summary.json"
        plot_path = output_dir / "ridge_forecast_plot.png"
    ...
    def _save_plot(series: pd.Series, forecast_df: pd.DataFrame, plot_path: Path) -> None:
        ...
        plt.title("Forecast Agent - Auto Ridge")
        plt.xlabel("Date")
        plt.ylabel("y")
        plt.legend()
        plt.tight_layout()
