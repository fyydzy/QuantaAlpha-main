...
| 类别          | 模型                         | 说明                                                     |
| ----------- | -------------------------- | ------------------------------------------------------ |
| 预测比选（共 9 个） | XGBoost、LightGBM、CatBoost  | 梯度提升树系列                                                |
|             | Random Forest              | 随机森林                                                   |
|             | LASSO、ElasticNet、Ridge     | 正则化线性回归                                                |
|             | SARIMAX                    | 季节自回归移动平均                                              |
|             | LSTM                       | 长短期记忆神经网络                                              |
| 大语言模型（LLM）  | 由 `.env` 中 `CHAT_MODEL` 指定 | 意图解析、特征推荐、完成后问答；需配置 `OPENAI_API_KEY`、`OPENAI_BASE_URL` |


Web 智能体默认以 `auto` 模式**依次跑完上述 9 个预测模型**并比选最优，约需 **10 分钟**，进度条期间请勿关闭页面或后端。

导航入口：**「今冬明春预测」**

### 2. 天气预测（ECMWF 季节气温）

**数据源**：数据集 [Seasonal forecast daily and subdaily data on single levels](https://cds.climate.copernicus.eu/)，2m 温度、6 小时步长，最长约 7 个月（215 天）。该数据为集合预报，每个时刻会给出 **51 套预报成员**（`number=0..50`）。当前脚本默认起报日 **2026-05-01**，区域为河北代表框（石家庄附近）。

- 查看 **ECMWF S5 季节预报** 导出的河北代表点气温数据
- **左栏**：6 小时步长预览，按北京日期 + 整点查
- **右栏**：日度气温
- 数据文件放在 `data/weather/`
...
| 多模型比选很慢             | 正常现象；`auto` 会依次跑 9 个模型（含 LSTM），全部完成约 **10 分钟**                                   |
...
QuantaAlpha-main/
├── configs/                              # 配置文件
│   ├── forecast.yaml                     # 燃气预测 CLI 默认参数
│   └── .env.example                      # 环境变量模板（亦可参考根目录）
├── data/
│   ├── processed_data/                   # 省级燃气历史 Excel（{省份}.xlsx）
│   │   └── README.md                     # 数据格式说明
│   └── weather/                          # ECMWF 气温 CSV（天气预测页）
├── docs/
│   ├── forecast.md                       # 燃气预测详细说明
│   ├── weather_forecast.md               # 气温数据下载与处理
│   └── 第三版修改方案.md                  # 对话式智能体设计
├── forecast_agent_output/                # 预测结果输出（按省份 / 任务分目录）
│   └── {省份}/
│       └── agent_flow_YYYYMMDD_HHMMSS/   # 单次 Web 智能体任务
├── frontend-v2/                          # Web 前后端
│   ├── src/
│   │   ├── pages/
│   │   │   ├── ForecastPage.tsx          # 今冬明春预测对话页
│   │   │   └── WeatherForecastPage.tsx   # 天气预测页
│   │   ├── context/TaskContext.tsx       # 任务状态与 WebSocket
│   │   └── services/api.ts               # 后端 API 封装
│   └── backend/
│       └── app.py                        # FastAPI 后端（燃气智能体 + 天气 API）
├── quantaalpha/
│   └── forecast_agent/                   # 燃气预测核心
│       ├── gas_forecast_flow.py          # 智能体编排与 LLM 提示词
│       ├── runner.py                     # 多模型比选调度
│       ├── cli.py                        # 命令行入口
│       ├── *_agent.py                    # 各预测模型实现（xgboost、lstm 等）
│       └── flow/                         # 流程阶段与状态
